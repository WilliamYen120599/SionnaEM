#!/usr/bin/env python3
"""Smoke-test pipeline on a known Sionna-RT built-in scene."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Verify a known scene RT pipeline.")
    p.add_argument("--tx", nargs=3, type=float, default=(0.0, 0.0, 2.0),
                   help="TX position x y z, e.g. 0 0 2")
    p.add_argument("--rx", nargs=3, type=float, default=(-2.0, 0.0, 2.0),
                   help="RX position x y z, e.g. -2 0 2")
    ...
    p.add_argument("--out-prefix", default="floor_wall_pipeline", help="Output file prefix")
    p.add_argument("--scene-name", default="floor_wall", help="Output folder name under --out-dir")
    return p.parse_args()
...
def main() -> None:
    ...
    # Load known Sionna built-in scene
    ...
    print(f"[pass] Loaded scene: {sionna.rt.scene.floor_wall}")
    ...
    np.savez_compressed(npz_out, a=a, tau=tau, tx=args.tx, rx=args.rx,
                        n_paths=n_paths, total_power=total_power, solve_time_ms=t_rt * 1e3)

    plot_path = out_dir / f"{args.out_prefix}_path_powers.png"
    save_path_power_plot(plot_path, power, "floor_wall", tau0)
